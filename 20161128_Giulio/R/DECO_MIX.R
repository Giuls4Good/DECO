...
DECO_LASSO_MIX <- function(Y, X, p=NULL, n=NULL, m=1, lambda, r_1, r_2 = r_1, ncores=1, intercept=TRUE, refinement = TRUE){
  ...
  Xi <- mclapply(1:m,
                 function(i){
                   ...
                   startGroup <- floor(p/m)*(i-1) + 1 #Same splitting  fashion used in the C++ functions
                   if(i < m) {
  ...
  XX_Inverse <- invSymmMatrix(XX + r*diag(n)); rm(XX);gc() #obtain the inverse (X'X + rI)^(-1) and save memory
